    ...
    <main className="bg-[#212529] h-screen overflow-y-auto p-6">
      ...
      <div className={`flex items-center mt-4 gap-4 ${clickOpencategory} overflow-x-auto`}>
        {data.map((category, index) => (
          <button key={index} className="flex-none border-2 border-[#03FFAB] w-fit h-fit p-1 rounded hover:bg-[#03FFAB] focus:bg-[#03FFAB] text-sm text-[#03FFAB] focus:text-black hover:text-black">
            {category.category}
          </button>
        ))}
      </div>
      ...
      <div className="mt-4 gap-12 sm:flex sm:flex-wrap sm:justify-center">
        {data.map((item, index) => (
          <div key={index} className="flex flex-col gap-4 mb-4 sm:items-start sm:w-fit">
            <h3 className="text-lg font-bold text-white w-full justify-start">{item.category}</h3>
            <div className="flex gap-4 overflow-x-auto">
              {item.agents.map((agent, agentIndex) => (
                <div key={agentIndex} className="flex items-center flex-col flex-none border-2 rounded-[30px] w-[170px] h-[300px] p-2 bg-[#697179]">
